...
const VistaClientePerfil = () => {
    ...
    const handleAgregarDireccionClick = () => {
        setMostrarFormularioDireccion(true);
    };

    const handleCambiarDireccion = (e) => {
        setNuevaDireccion(e.target.value);
    };

    const handleGuardarDireccion = () => {
        if (!nuevaDireccion || !latitud || !longitud || !codigoPostal) {
        ...
        axios.post('http://localhost:8000/direcciones-envio', nuevaDireccionData)
            .then(response => {
            ...
            .catch(error => {
                ...
                alert("Hubo un error al agregar la dirección. Intenta de nuevo.");
            });
    };
                    ...
                    <div className="perfil-direcciones">
                        ...
                        {direcciones.length > 0 ? (
                            direcciones.map((direccion) => (
                                <div key={direccion.id} className="perfil-item-direccion">
                                    <p>{userData.nombre}</p>
                                    <p>{direccion.otro}</p>
                                    <p>{direccion.latitud}</p>
                                    <p>{direccion.longitud}</p>
                        ...
                        <button className="perfil-agregar-direccion" onClick={handleAgregarDireccionClick}>+</button>
                    </div>

                    {mostrarFormularioDireccion && (
                        <div className="perfil-agregar-direccion-formulario">
                            <h4>Agregar nueva dirección</h4>
                            <input
                                type="text"
                                value={nuevaDireccion}
                                onChange={handleCambiarDireccion}
                            ...
                            />
                            <div className="perfil-agregar-direccion-botones">
                                <button onClick={handleGuardarDireccion} style={{ backgroundColor: '#4CAF50', color: 'white', marginRight: '10px' }}>Guardar</button>
                                <button onClick={handleCancelarDireccion} style={{ backgroundColor: '#f44336', color: 'white' }}>Cancelar</button>
                            </div>
                        </div>
                    )}
                </div>

                <div className="perfil-tarjetas">
                    <h3>Tarjetas de Crédito</h3>
                    {tarjetasCredito.length > 0 ? (
                        tarjetasCredito.map((tarjeta) => (
                            <div key={tarjeta.id} className="perfil-item-tarjeta">
                                <p><strong>Tipo:</strong> {tarjeta.tipoTarjeta}</p>
                                <p><strong>Titular:</strong> {tarjeta.nombreTitular}</p>
                                <p><strong>Número de tarjeta:</strong> ****** {tarjeta.numeroTarjeta.slice(-4)}</p>
                                <p><strong>Fecha de vencimiento:</strong> {tarjeta.fechaVencimiento}</p>
                            </div>
                        ))
                    ) : (
                        <p>No tienes tarjetas de crédito asociadas.</p>
                    )}
                    <button className="perfil-agregar-tarjeta" onClick={() => setMostrarAgregarTarjeta(true)}>+</button>
                </div>
...
export default VistaClientePerfil;